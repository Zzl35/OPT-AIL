...
def load_trajectories(expert_location: str,
    ...
    if os.path.isfile(expert_location):
        ...
        with open(expert_location, 'rb') as f:
            data = read_file(expert_location, f)
        trajs = {}
        trajs["states"] = data[1][:10, :499, :]
        trajs['actions'] = data[2][:10, :499, :]
        trajs["next_states"] = data[1][:10, 1:, :]
        trajs['rewards'] = data[3][:10, :499]
        trajs['dones'] = np.zeros_like(trajs['rewards'])
        trajs['lengths'] = np.ones((10, 1), dtype=np.int32) * 499
